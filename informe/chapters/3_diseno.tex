...
\todo[inline]{Describir brevemente el diseño}

\todo[inline]{Agregar listado de mensajes y servicios en los anexos.}

\subsubsection{Formato y representación}

% SOBRE ELECCIÓN DE MENSAJES DE ROS
Se decidió utilizar mensajes de ROS para la representación de episodios. Esto se justifica en la amplia gama de mensajes base que provee ROS, los que permiten construir un mensaje adecuado a cualquier necesidad. Además, utilizando éste sistema de mensajes, se evita definir una formato de información particular para la representación episódica, y la comunicación de episodios mediante la API ROS del servidor se simplifica. Más aún, como se explicará en más detalle en la Sección \ref{}, el driver para MongoDB a utilizar permite almacenar cualquier estructura de datos, mientras ésta esté encapsulada en un mensaje ROS.

% SOBRE UNICIDAD
El requisito de software \hl{(RSXX)} exige que cada episodio sea único. Para esto, el mensaje ROS debe contener un id único para identificar el episodio. El servidor debe encargarse de que cada nuevo episodio cuente con un identificador único.

% SOBRE CAMPO DE TAGS
Además, para simplificar la búsqueda y reconocimiento de episodios, se incluye un campo de \textit{tags}. Éste campo corresponde a un vector de \textit{strings} y permite almacenar descripciones breves de la naturaleza del episodio. Por ejemplo, un episodio podría contener los siguientes \textit{tags}: ``cumpleaños'', ``fiesta'', ``atender invitados'' y ``torta''.
...
\unsure[inline]{Los episodios deben ser indexados por ambos campos... Mejor digo esto en la parte de la base de datos.}

 
\subsubsection{Árboles de episodios}

\todo[inline]{Quitar manejo de contexto del trabajo??... no se si vale la pena que el server maneje ese concepto. Puede ser trabajo futuro.}

\unsure[inline]{Es correcto hablar de hojas y nodos??... Debo buscar una palabra más adecuada para nodos que no son hojas??.. o debo explicitar que en adelante, cualquier referencia a nodo se refiere a eso. Suena raro hablar de ``padres''.}
...
\todo[inline]{Se puede agregar e implementar la condición de que hijos son ordenados por tiempo de inicio}
 
\subsubsection{Contexto temporal: \textit{When}}
...
\todo[inline]{Revisar o modificar implementación.. no estoy seguro como se están manejando los tiempos para los padres... lo lógico sería no ajustar los tiempos a los de los hijos, pues estos pueden no estar definidos hasta el final!... O agregarlo como limitante.}
En el caso de las hojas del árbol, basta utilizar los instantes de tiempo obtenidos al ingresar el episodio. Sin embargo, para el caso de los \hl{nodos}, se pueden seguir dos estrategias: Utilizar los instantes obtenidos al ingresar el episodio, o calcular los tiempos de inicio y fin para ajustarse a los hijos. A pesar de que la segunda opción suena razonable, está asumiendo que siempre existirá un hijo para cada instante de tiempo de vida del padre, lo que puede no ser cierto. Por lo tanto, se deben manejar cuidadosamente los valores de tiempo de los padres, para asegurar que no son sobre-escritos por sus hijos, y para asegurar que siempre inician antes y terminan después que ellos.
...
% SOBRE LOS REQUISITOS
De acuerdo al requisito de software \hl{(RSXX)}, los episodios deben almacenar información que indique su contexto espacial (\textit{Where}), es decir, dónde sucedió. 


% SOBRE WHERE

Frames, mapa, 2D y robots de servicio.
Convex hull 2D
Ejemplo casa vs cocina
Manejo de campo en padres e hijos.. position se refiere al centroide del convex hull
Datos de hijos


% SOBRE MENSAJE EPISODICO
Considerando el diseño estudiado, se crea el mensaje \texttt{Where.msg} para almacenar el contexto espacio del episodio. Se consideran los siguientes campos:
\begin{itemize}
