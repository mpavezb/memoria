...
El otro foco del trabajo está en el cumplimiento de las reglas episódicas propuestas por Stachowicz. A modo de resumen, todas las validaciones asociadas se cumplen, a excepción de \Vlabel{A}{17}. A continuación se describen los conceptos asociados a cada una de las validaciones de interés.
...
\todoNOPE{Código para ejecutar el sistema simulado + consultas interesantes.}
...
\todoNOPE{Código de ejemplo para filtrar por relevancia.}
...
\ltmconcept{Plugin para entidades} Se implementó un plugin para la recopilación de información facial sobre humanos. Todos los datos son obtenidos a partir del sistema de percepción del robot Bender. Esto se puede validar mediante la demostración implementada durante la integración.
...
El robot objetivo de este trabajo es Bender, el que se entiende como un robot de laboratorio, con un tiempo de operación diario muy bajo. Se tienen las siguientes consideraciones:
\begin{itemize}
...
\todoNOPE{J.Simmonds: $Q_4$ no se ve tan distinta de $Q_5$... que crees que ocurre con esta consulta, porque se ve este comportamiento?}
% sobre costo de consultas AND/OR... orden de sub queries...   
...
\todoNOPE{Coma decimal, punto miles.}
